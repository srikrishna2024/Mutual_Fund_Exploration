...
def calculate_portfolio_metrics(weights_df, returns_df):
    ...
    # Calculate yearly metrics (annualized from monthly)
    yearly_fund_volatilities = monthly_fund_volatilities * np.sqrt(12)
    
    # Calculate portfolio variance and std dev (monthly)
    ...
    # Calculate yearly portfolio metrics
...
def main():
    ...
    try:
        ...
        # [Previous code remains exactly the same until the Correlation Matrix section]
        ...
        **Interpretation Guidelines:**
        1. **Diversification Assessment:**
           - Lower correlations (closer to 0 or negative) indicate better diversification
           - High correlations (>0.70) suggest funds may be exposed to similar risks
        
        2. **Portfolio Implications:**
           - Funds with high correlation may not provide effective diversification
           - Consider replacing highly correlated funds with less correlated alternatives
           - Negative correlations can help reduce portfolio volatility
        
        3. **Risk Management:**
           - During market stress, correlations often increase
           - A mix of low and negatively correlated assets can provide better protection
           - Watch for funds that seem to move in lockstep during both up and down markets
        """)

        # [Previous code remains the same until the correlation warning message]
        ...
        if avg_correlation > 0.6:
            ...
            for fund in correlation_display.index:
                # Calculate average correlation with other funds, excluding self-correlation
            ...
            # Identify highly correlated funds with significant weights
            ...
            # Identify funds with lower correlations
            ...
            # [Previous code remains the same until the rebalancing suggestions section]

            st.subheader("Portfolio Rebalancing Suggestions")
            
            # Display suggestions for reducing exposure
            if not high_corr_funds.empty:
                st.write("**Funds to Consider Reducing Exposure:**")
                
                # Create DataFrame for high correlation funds
                reduce_exposure_data = []
                for fund in high_corr_funds.index:
                    # Get top 3 correlated funds
            ...
            **Rebalancing Guidelines:**
            ...
            suggested_reduction = high_corr_funds['weight'].sum() * 0.3  # Suggest reducing highly correlated positions by 30%
...
# [Rest of the code remains the same]
            ...
            current_portfolio_corr = avg_correlation
            suggested_reduction = high_corr_funds['weight'].sum() * 0.3  # Suggest reducing highly correlated positions by 30%
            potential_improvement = f"""
            **Potential Impact of Rebalancing:**
            - Current portfolio average correlation: {current_portfolio_corr:.2f}
            - Suggested reduction in highly correlated funds: {suggested_reduction:.1f}% of portfolio
            - Target: Reduce average portfolio correlation below 0.60
            """
            st.write(potential_improvement)
...
# [Rest of the code remains the same]
        ...
        # Fund-wise volatility comparison
        st.subheader("Fund-wise Volatility (Yearly)")
        vol_comparison = pd.DataFrame({
            'Fund': weights_df['scheme_name'],
            'Weight (%)': weights_df['weight'],
            'Monthly Volatility (%)': weights_df['code'].map(risk_metrics['monthly_fund_volatilities'] * 100),
            'Yearly Volatility (%)': weights_df['code'].map(risk_metrics['yearly_fund_volatilities'] * 100),
            'Current Value': weights_df['current_value'].apply(format_indian_number)
        }).sort_values('Yearly Volatility (%)', ascending=False)
        
        st.dataframe(vol_comparison.style.format({
            'Weight (%)': '{:.2f}',
            'Monthly Volatility (%)': '{:.2f}',
            'Yearly Volatility (%)': '{:.2f}'
        }))
        ...
        with col2:
            ...
            # Create a mapping of code to scheme name
            code_to_scheme = weights_df.set_index('code')['scheme_name'].to_dict()
            
            # Find best and worst performing funds using scheme names
