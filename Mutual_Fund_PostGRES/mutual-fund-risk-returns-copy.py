...
class MutualFundRiskReturnsAnalyzer:
    ...
    def calculate_fund_metrics(self, category, time_period):
        ...
        try:
            ...
            # Corrected SQL query with NULLIF for safe division
            query = """
            WITH fund_nav AS (
                SELECT 
                    md.scheme_code, 
                ...
                SELECT 
                    scheme_code,
                    date,
                    (net_asset_value / NULLIF(LAG(net_asset_value) OVER (PARTITION BY scheme_code ORDER BY date), 0) - 1) * 100 AS daily_return,
                    net_asset_value
                FROM fund_nav
            ), daily_benchmark_returns AS (
                SELECT 
                    date,
                    (price / NULLIF(LAG(price) OVER (ORDER BY date), 0) - 1) * 100 AS daily_return,
                    price
                FROM benchmark_nav
            ), fund_aggregates AS (
                SELECT 
                    scheme_code,
                    (MAX(net_asset_value) / NULLIF(MIN(net_asset_value), 0) - 1) * 100 AS fund_return,
                    COALESCE(STDDEV(daily_return), 0) AS fund_std_dev
                FROM daily_fund_returns
                GROUP BY scheme_code
            ), benchmark_aggregates AS (
                SELECT 
                    (MAX(price) / NULLIF(MIN(price), 0) - 1) * 100 AS benchmark_return,
            ...
            SELECT 
                fa.scheme_code,
                mf.scheme_name,
                COALESCE(fa.fund_return, 0) AS fund_return,
                COALESCE(fa.fund_std_dev, 0) AS fund_std_dev,
                COALESCE(ba.benchmark_return, 0) AS benchmark_return,
                COALESCE(ba.benchmark_std_dev, 0) AS benchmark_std_dev,
                COALESCE(fa.fund_return - ba.benchmark_return, 0) AS xirr_diff,
...
def main():
    ...
    if analyze_button:
        ...
        # Create quadrant scatter plot with log scale
        ...
        fig.add_shape(type="line", x0=1, x1=1, y0=0.1, y1=100, 
                      line=dict(color="Red", width=2, dash="dash"))
        fig.add_shape(type="line", x0=0.1, x1=100, y0=1, y1=1, 
                      line=dict(color="Red", width=2, dash="dash"))
